...
void iotjs_tizen_app_control_cb(app_control_h app_control, void* user_data) {
  ...
  // call emit
  jerry_value_t jargv[2] =
      { jerry_create_string_from_utf8((const jerry_char_t*)event_name),
        jerry_create_string_from_utf8((const jerry_char_t*)json) };

  iotjs_make_callback(fn, tizen, jargv, 2);
...
static bool bridge_native_call(const char* module_name, const char* func_name,
  ...
  jerry_value_t jmessage =
      jerry_create_string_from_utf8((const jerry_char_t*)message);
  jerry_value_t jres =
      iotjs_make_callback_with_result(jfunc, jmodule, &jmessage, 1);
