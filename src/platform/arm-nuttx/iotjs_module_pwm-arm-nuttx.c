...
static inline uint32_t iotjs_pwm_get_timer(uint32_t pin) {
  return (pin >> TIMER_PIN_SHIFT) & TIMER_PIN_MASK;
}
...
void ExportWorker(uv_work_t* work_req) {
  ...
  uint32_t timer = iotjs_pwm_get_timer(pin);
  char path[PWM_DEVICE_PATH_BUFFER_SIZE] = { 0 };
  iotjs_pwm_get_path(path, timer);
...
void SetFrequencyWorker(uv_work_t* work_req) {
  ...
  uint32_t timer = iotjs_pwm_get_timer(req_data->pin);
...
void UnexportWorker(uv_work_t* work_req) {
  ...
  iotjs_pwm_unconfig_nuttx(req_data->pin);
