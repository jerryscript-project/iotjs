...
static bool iotjs_spi_set_configuration(iotjs_spi_t* spi) {
  ...
  int fd = _this->platform->device_fd;
  if (fd < 0) {
    return false;
  }
...
bool iotjs_spi_transfer(iotjs_spi_t* spi) {
  ...
  // Transfer data
  int err = ioctl(_this->platform->device_fd, SPI_IOC_MESSAGE(1), &data);
  if (err < 1) {
...
bool iotjs_spi_close(iotjs_spi_t* spi) {
  ...
  if (_this->platform->device_fd >= 0) {
    const iotjs_environment_t* env = iotjs_environment_get();
    uv_loop_t* loop = iotjs_environment_loop(env);
    uv_fs_t fs_close_req;

    int err =
        uv_fs_close(loop, &fs_close_req, _this->platform->device_fd, NULL);
    uv_fs_req_cleanup(&fs_close_req);
    if (err < 0) {
      DLOG("%s - close failed: %d", __func__, err);
      return false;
    }
    _this->platform->device_fd = -1;
  }
...
void iotjs_spi_open_worker(uv_work_t* work_req) {
  ...
  const char* device_path = iotjs_string_data(&_this->platform->device);
  if (iotjs_systemio_check_path(device_path)) {
    ...
    _this->platform->device_fd = open_req.result;
...
void iotjs_spi_platform_create(iotjs_spi_t_impl_t* _this) {
  ...
  _this->platform->device = iotjs_string_create("");
}

void iotjs_spi_platform_destroy(iotjs_spi_t_impl_t* _this) {
  iotjs_string_destroy(&_this->platform->device);
...
void iotjs_spi_platform_set_cofiguration(iotjs_spi_t_impl_t* _this,
  ...
  _this->platform->device = iotjs_jval_as_string(&jdevice);
