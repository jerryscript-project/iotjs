...
void Complete(napi_env env, napi_status status, void* data) {
  ...
  napi_value result;
  NAPI_CALL_RETURN_VOID(env, napi_call_function(env, global, callback, 2, argv,
                                                &result));
...
napi_value Test(napi_env env, napi_callback_info info) {
  ...
  NAPI_ASSERT(env, t == napi_number, "Wrong first argument, integer expected.");
  NAPI_CALL(env, napi_typeof(env, argv[1], &t));
  NAPI_ASSERT(env, t == napi_object, "Wrong second argument, object expected.");
  ...
  NAPI_CALL(env, napi_create_string_utf8(env, "TestResource", NAPI_AUTO_LENGTH,
                                         &resource_name));
  NAPI_CALL(env, napi_create_async_work(env, argv[1], resource_name, Execute,
                                        Complete, &the_carrier,
...
void CancelComplete(napi_env env, napi_status status, void* data) {
  ...
  if (status == napi_cancelled) {
    ...
    napi_value callback;
    NAPI_CALL_RETURN_VOID(env, napi_get_reference_value(env, c->_callback,
                                                        &callback));
    napi_value global;
    NAPI_CALL_RETURN_VOID(env, napi_get_global(env, &global));
    napi_value result;
    NAPI_CALL_RETURN_VOID(env, napi_call_function(env, global, callback, 0,
                                                  NULL, &result));
  }
...
napi_value TestCancel(napi_env env, napi_callback_info info) {
  ...
  for (int i = 1; i < MAX_CANCEL_THREADS; i++) {
    NAPI_CALL(env,
              napi_create_async_work(env, NULL, resource_name, CancelExecute,
                                     BusyCancelComplete, &async_carrier[i],
                                     &async_carrier[i]._request));
  ...
  // workers above.
  NAPI_CALL(env, napi_get_cb_info(env, info, &argc, argv, &_this, &data));
  NAPI_CALL(env, napi_create_async_work(env, NULL, resource_name, CancelExecute,
                                        CancelComplete, &async_carrier[0],
                                        &async_carrier[0]._request));
  NAPI_CALL(env, napi_create_reference(env, argv[0], 1,
                                       &async_carrier[0]._callback));
...
static void RepeatedWorkComplete(napi_env env, napi_status status, void* data) {
  ...
  NAPI_CALL_RETURN_VOID(env,
                        napi_get_reference_value(env, repeated_work_info.ref,
                                                 &cb));
  NAPI_CALL_RETURN_VOID(env,
...
static napi_value DoRepeatedWork(napi_env env, napi_callback_info info) {
  ...
  NAPI_CALL(env, napi_create_string_utf8(env, "Repeated Work", NAPI_AUTO_LENGTH,
                                         &name));
  NAPI_CALL(env,
